...
const
  //область слева:
  DmTxt_RightAreaFromBound: boolean = false;//ОБЛАСТЬ СЛЕВА !!!
...
type
  ...
  public
    ...
    constructor CreateStr(aLcode: longint; aStr: string); //Point[0]=(0;0)
...
type
  ...
  TxAlignment = (txaLeft{0}, txaRight{1}, txaCenter{2}, txaDouble{3});
...
type
  ...
  tt_data = record//uses in LoadFromRichEdit
    ...
    Interval_mm: double;
    Indent_mm: double;//отступ от границы области
    FirstIndent_mm: double;//"красн строка", "<0" => выступ
  end;
  pt_data = ^tt_data;
...
*)
  ...
  public
    ...
    function LoadFromRichEdit(RE: TRichEdit; PixelsPerInch: Integer; lpmm: double; p_data: pt_data): boolean;//Clear(!)
    procedure LoadFromRichEdit_Grunt(RE: TRichEdit; PixelsPerInch: Integer; lpmm: double; acode_t,acode_tb,acode_ti: longint);//Clear(!)
    ...
    //HorizText=FALSE => текст против направления последнего отрезка области (область против ч.стр.):
    procedure AddToPabPaeFromArea(area0: tpl; pab,pae: tpa; lInterval,lIndent,debcode: longint;
                          HorizText: boolean = TRUE);
...
const char_for_height: pChar = 'X'; //WIN
...
begin
  if lcode<=0 then begin
    Tell('dmtxt_old.DmTxt_Box: lcode=0');
  end;
...
*)
    ...
    //шаг штриховки:
    if HorizText
    then dp := v_xy( 0, lInterval )
    else dp := v_rot( v_norm(area.V[area.count-2], lInterval), -90 );//последний отрезок
...
begin
  ...
  pae:=tpa.New;
  try
    AddToPabPaeFromArea(area0, pab,pae, lInterval,lIndent,debcode, HorizText);
...
function TDmText.LoadFromRichEdit(RE: TRichEdit; PixelsPerInch: Integer; lpmm: double; p_data: pt_data): boolean;
...
var
  ...
  dmpara: TDmPara;
  dmword: tdmstr;
  oldwordwrap,codes_ok: boolean;
//  xx,yy: integer;//DEBUG
begin
  Result:=TRUE;
  if p_data^.code_t<=0 then begin
    Tell('Код основного текста = 0 - EXIT');
    Result:=FALSE;
    EXIT;//!
  end;
  ...
  with p_data^ do begin
    if code_tb<=0 then begin code_tb:=code_t; codes_ok:=false; end;
    if code_ti<=0 then begin code_ti:=code_t; codes_ok:=false; end;
    if code_tu<=0 then begin code_tu:=code_t; codes_ok:=false; end;
  end;
  if not codes_ok then Tell('Часть кодов = 0 - Заменены на основной');
...
//      yy:=RE.Paragraph.RightIndent;
    ...
    while true do begin
      ...
      dmword:=tdmstr.CreateStr(p_data^.code_t,sword);
      //Bold,Italic,Underline:
      if fsBold in RE.SelAttributes.Style then dmword.lcode:=p_data^.code_tb;
      if fsItalic in RE.SelAttributes.Style then dmword.lcode:=p_data^.code_ti;
      if fsUnderline in RE.SelAttributes.Style then dmword.lcode:=p_data^.code_tu;
...
procedure TDmText.LoadFromRichEdit_Grunt(RE: TRichEdit; PixelsPerInch: Integer; lpmm: double; acode_t,acode_tb,acode_ti: longint);//Clear(!)
var t_data: tt_data;
begin
