  ...
  try
    if DMWGetWindowGauss(x1,y1,x2,y2) then begin
      ...
      if FBlkCount>0 then for i:=0 to FBlkCount-1 do begin
        ...
        dmwlib_draw_rectgauss(win.a,win.b, 2, 3);
...
var i: integer; x1,y1,x2,y2: double; p: tnum2; win: tnum4; pl: tpl;
begin
  if not FOk then begin Tell('Файл точек не открыт'); exit; end;//!
  ...
  try
    ...
    dmwlib_draw_plgauss(pl, 2, 5);//перед pl.Free!
    pl.Free;
    SetDefCursor;
    RunForm.Finish;
  end;
