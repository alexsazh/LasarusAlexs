...
function xOpenStorage(Path: PChar; Mode: int; out stg: IStorage): HResult;
...
procedure xStmToFile(src: IStream; dst: THandle);
procedure xFileToStm(src: THandle; dst: IStream);
...
var
  fn: TWideStr;
begin
  Result:=false; if FileExist(Path) then
  if StrToWideChar(Path,fn,255) <> nil then
  Result:=StgIsStorageFile(fn) = S_OK
end;
...
begin
  ...
  if stg.Stat(statstg,0) = s_OK then begin
...
function xCreateStorage(Path: PChar;
                        out doc: IStorage): HResult;
var
  fn: TWideStr; mode: DWord;
begin
  Result:=s_FALSE; doc:=nil; mode:=ole_CREATE;

  if Path = nil then begin
    mode:=mode +STGM_DELETEONRELEASE;
    Result:=StgCreateDocFile(nil,mode,0,doc)
  end else
  if StrToWideChar(Path,fn,255) <> nil then
    Result:=StgCreateDocFile(fn,Mode,0,doc);

  if Result <> S_OK then doc:=nil
end;
...
function xOpenStorage(Path: PChar; Mode: Integer;
                      out stg: IStorage): HResult;
var
  fn: TWideStr;
begin
  Result:=S_False;
  if StrToWideChar(Path,fn,255) <> nil then
  Result:=StgOpenStorage(fn,nil,Mode,nil,0,stg)
end;
...
function xEnumFolders(stg: IStorage; Items: TStrings): Integer;
var
  stat: TStatStg; enum: IEnumStatStg;
  malloc: IMalloc; ic: Integer;
begin
  ...
  while enum.Next(1,stat,@ic) = s_OK do
  with stat do begin
...
begin
  ...
  if CoGetMalloc(MEMCTX_Task,malloc) = s_OK then
  if stg.Stat(statstg,0) = s_OK then begin
    if FileTimeToSystemTime(statstg.mtime,st) then
...
begin
  ...
  if CoGetMalloc(MEMCTX_Task,malloc) = s_OK then
  if sf.Stat(statstg,0) = s_OK then begin
...
begin
  ...
  if CoGetMalloc(MEMCTX_Task,malloc) = s_OK then
  if sf.Stat(statstg,0) = s_OK then begin
    dt.c[0]:=statstg.mtime.dwLowDateTime;
    dt.c[1]:=statstg.mtime.dwHighDateTime;
    Result:=dt.x; malloc.Free(statstg.pwcsName)
  end;
end;
...
begin
  ...
  if CoGetMalloc(MEMCTX_Task,malloc) = s_OK then
  if sf.Stat(statstg,0) = s_OK then begin
    Path:=statstg.pwcsName; if Assigned(Path) then
...
begin
  cx:=xSize(src); xSeek(src,0);

  bx:=0;
  while bx < cx do begin
    dx:=Min(cx-bx,sizeOf(buf));
    xRead(src,buf,dx);
    FileWrite(dst,buf,dx);
    Inc(bx,dx)
  end
end;
...
function TStorage.Open_Stream(Name: PChar; rw: Boolean; out stm: IStream): Boolean;
begin
