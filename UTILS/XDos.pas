...
begin
  ...
  if FileExist(fn) then begin
    ...
    h:=FileOpen(StrPas(fn),grfMode);

    if h > 0 then

    if grfMode and 15 = fmOpenRead then begin

      with TOnlyRead.Create(h) do
      Result:=QueryInterface(IStream,stm)
...
function TDosStorage.CopyTo(ciidExclude: Longint; rgiidExclude: PIID;
...
constructor TOnlyRead.Create(h: Integer);
begin
  inherited Create;

  vm_h:=CreateFileMapping(h,nil,PAGE_READONLY,0,0,nil);

  if vm_h <> 0 then
  vm_p:=MapViewOfFile(vm_h,FILE_MAP_READ,0,0,0);

  if h > 0 then begin
    vm_Size:=FileSeek(h,0,2);
    FileClose(h)
  end
  else vm_Size:=MaxInt
end;
...
begin
  if vm_p <> nil then UnmapViewOfFile(vm_p);
  if vm_h <> 0 then CloseHandle(vm_h);
  inherited
end;
...
function TOnlyRead.Read(pv: Pointer; cb: _DWORD; pcbRead: _PDWORD): HResult;
begin
...
function TOnlyRead.Write(pv: Pointer; cb: _DWORD; pcbWritten: _PDWORD): HResult;
begin
  Result:=E_NOTIMPL
end;
...
function TOnlyFile.Read(pv: Pointer; cb: _DWORD; pcbRead: _PDWORD): HResult;
begin
  pcbRead^:=FileRead(vm_h,pv^,cb);
  Result:=S_OK
end;

function TOnlyFile.Write(pv: Pointer; cb: _DWORD; pcbWritten: _PDWORD): HResult;
begin
  Result:=S_FALSE;
...
function TOnlyFile.Seek(dlibMove: Largeint; dwOrigin: Longint;
  out libNewPosition: Largeint): HResult;
begin
  libNewPosition:=FileSeek(vm_h,Int64(dlibMove),dwOrigin);
  Result:=S_OK
end;
...
begin
  ...
  if vm_rw then begin
    ...
    if Old < libNewSize then begin
      ...
      FileSeek(vm_H,Int64(libNewSize),0);
      _lwrite(vm_H,@vm_H,0);
    end;
...
function TCashFile.Read(pv: Pointer; cb: _DWORD; pcbRead: _PDWORD): HResult;
var
...
function TCashFile.Write(pv: Pointer; cb: _DWORD; pcbWritten: _PDWORD): HResult;
var
  buf: pbytes; len,ofs,pos: Integer;
begin
  Result:=S_FALSE;
...
function TCashFile.Seek(dlibMove: Largeint; dwOrigin: Longint;
  out libNewPosition: Largeint): HResult;
begin
  vm_Pos:=FileSeek(vm_h,Int64(dlibMove),dwOrigin);
...
begin
  ...
  if vm_rw then begin
    put_Page; FileSeek(vm_H,Int64(libNewSize),0);
    _lwrite(vm_H,@vm_H,0); Result:=S_OK
  end
end;
...
function TMemoryStorage.CopyTo(ciidExclude: Longint; rgiidExclude: PIID;
  snbExclude: TSNB; const stgDest: IStorage): HResult;
begin
  Result:=E_NOTIMPL
end;
...
function TOnlyMemory.Read(pv: Pointer; cb: _DWORD; pcbRead: _PDWORD): HResult;
var
  len: Longint;
begin
...
function TOnlyMemory.Write(pv: Pointer; cb: _DWORD; pcbWritten: _PDWORD): HResult;
var
  len,size: Integer;
begin
