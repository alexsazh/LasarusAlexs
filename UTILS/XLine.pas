...
function iTransit_3x3p(const p: TPoint; const T: Real3x3): TPoint;
function gTransit_3x3p(const p: TPoint; const T: Real3x3): TGauss;
...
procedure float_3x3(var T: Real3x3);

function swap_3x3(const T: Real3x3): Real3x3;

function Get_scale_3x3(const T: Real3x3): Double;

function Jpeg_3x3(ix,iy,k,ox,oy: double; pbt: PReal3x3): Real3x3;

procedure vec_scale(X: PDoubles; N: int; k: Double);

function Norm_Degree(Fi: int): int;
...
function inter_point(const p1,p2: TPoint; r1,r2,r: Double): TPoint;
...
function gauss_forw(const p,v: tgauss; r: double): tgauss;
function point_forw(const p,d: TPoint; r: double): TPoint;

function point_dir(const a,b: TPoint): TPoint;

procedure Get_F_Point(x,y: int; a,b,f: double; out p: TPoint);

procedure Get_tube(ax,ay,bx,by,cx,cy, r1,r2: Double;
                   out lx,ly,rx,ry: Double);

procedure Get_Tube_Points(const a,b,c: TPoint;
                          r1,r2: Double;
                          out b1,b2: TPoint);

function Get_Left_Points(const a,b,c: TPoint;
                         r: Double; lp: PLPoly): int;
...
function Get_Next_Point(const a,b: TPoint; r: double; out p: TPoint): Boolean;
...
function Get_Norm_Point(const a,b: TPoint;
                        d,h: double; out p: TPoint): Boolean;
...
function Long_vec(const a,b: TPoint; out v: TGauss): bool;
...
function iGet_dir(const a,b: TPoint; out v: TGauss): bool;
...
function c_pull_to_line1(const l: TLine_coeff;
                         var p: TGauss;
                         eps1,eps2: Double): Boolean;
...
function Plot_to_Line1(const p ,q1,q2: TPoint; out pc: TPoint): int;

function Plot_to_Line2(const p ,q1,q2,q3: TPoint): Boolean;

function Plot_to_XLine(const p,q1,q2: TGauss; out cp: TGauss): Boolean;
...
function i_zigzag(const p1,p2,p3,p4: TPoint): Boolean;
...
procedure Rotate_LPoint(const a,b,c, p: TPoint; out q: TPoint);
...
procedure Rotate_Gauss(const a,b,c, p: tgauss; var q: tgauss);
procedure Mirror_Gauss(const p,c: tgauss; var q: tgauss);

function Rotate_Point(const a,b,c: TGauss; const p: TPoint): TPoint;
...
function LineContainsEdge(const Aa,Ab: TPoint;
                          const max_r: Double;
...
function PortContainsPoint(const lt,rb: TPoint; x,y: int): Boolean;
function FrameContainsPoint(const lt,rb: TPoint; x,y,r: int): Boolean;

function PortContainsRect(const lt,rb: TPoint; x1,y1,x2,y2: int): Boolean;
function PortContainsPort(const lt,rb, _lt,_rb: TPoint): Boolean;
function PortContainsLine(const lt,rb, a,b: TPoint): Boolean;

function BoundContainsRect(const lt,rb: TPoint; x1,y1,x2,y2: int): Boolean;
...
function Clip_GPoly(lp: PGPoly; n: int; lx,rx: double): int;
...
function yClip_Line(w,h: int; const a,b: TPoint;
                    out c,d: TPoint): Boolean;
...
function iTransit_3x3p(const p: TPoint; const T: Real3x3): TPoint;
begin
  Result:=iTransit_3x3(p.X,p.Y,T)
end;

function gTransit_3x3p(const p: TPoint; const T: Real3x3): TGauss;
begin
  Result:=Transit_3x3(p.X,p.Y,T)
end;
...
var
  tr,bt: Real3x3;
begin
  Init_3x3(tr,-ix,-iy,1,1);
  t_Scale_3x3(tr,k);
  t_Move_3x3(tr,ox,oy);

  if Assigned(pbt) then begin
    Init_3x3(bt,-ox,-oy,1,1);
    t_Scale_3x3(bt,1/k);
    t_Move_3x3(bt,ix,iy);
    pbt^:=bt
  end;
...
var
  vx,vy,l: Double;
begin
  vx:=d.X; vy:=d.Y; l:=Hypot(vx,vy);
  if l > Small then begin
    vx:=vx / l; vy:=vy / l;
  end;

  Result.x:=p.x + Round(vx*r);
  Result.y:=p.y + Round(vy*r);
end;
...
function Stretch_line(x1,y1,x2,y2, r: double;
...
function Get_left_side(ax,ay,bx,by,cx,cy, r: Double;
                       pp: PGauss): int;
var
  x1,y1, x2,y2, x3,y3,x4,y4, vx,vy,r1: Double; c: tgauss;
begin
  Result:=1;
  pp.x:=bx; pp.y:=by;

  if not Stretch_line(ax,ay,bx,by, r, x1,y1,x2,y2) then begin
    if Stretch_line(bx,by,cx,cy, r, x3,y3,x4,y4) then begin
      pp.x:=x3; pp.y:=y3
    end
  end else

  if not Stretch_line(bx,by,cx,cy, r, x3,y3,x4,y4) then begin
    pp.x:=x2; pp.y:=y2
  end
  else begin
    pp.x:=x2/2 + x3/2;
    pp.y:=y2/2 + y3/2; r:=Abs(r);

    if Hypot(x3-x2,y3-y2) > r/2 then
    if not LL2(x1,y1,x2,y2, x4,y4,x3,y3, 0,10, c) then begin
    end
    else begin
      pp.x:=c.x; pp.y:=c.y;
      ...
      vx:=c.x-bx; vy:=c.y-by;
      r1:=Hypot(vx,vy);
      if r1 > r*1.5 then begin
        r1:=r*1.2/r1;
        c.x:=bx + vx*r1;
        c.y:=by + vy*r1;

        if Hypot(c.x-x2,c.y-y2) > r/3 then begin
          pp.x:=x2; pp.y:=y2; Inc(pp); Inc(Result);
          pp.x:=c.x; pp.y:=c.y; Inc(pp); Inc(Result);
          pp.x:=x3; pp.y:=y3;
        end               
      end
    end
...
var
  i,n: int; g: TGauss3;
begin
  n:=Get_left_side(a.X,a.Y,b.X,b.Y,c.X,c.Y, r,@g);
...
begin
  Result:=false;
  if LL2ic(p1,p4, p2,p3,c) then
  if int_cos3(p1,p2,p3,1,cos) then
//Result:=cos < 0
end;
...
begin
  ...
  if ab > Small then begin
    ...
    dr:=(a*x + b*y + c) / len;
    if Abs(dr) <= eps1 then begin

      eps2:=eps2/len;

      if Abs(tk) < eps2 then tk:=0 else
      if Abs(1-tk) < eps2 then tk:=1;
...
begin
  ...
  if (p1.X < x2) and (p2.X > x1) then
  if (p1.Y < y2) and (p2.Y > y1) then begin
    ...
    if dx = 0 then begin
      if (y1 >= p1.Y-r) and (y2 <= p2.Y+r) then
      begin c.X:=a.X; Result:=true end
    end else
    if dy = 0 then begin
      if (x1 >= p1.X-r) and (x2 <= p2.X+r) then
      begin c.Y:=a.Y; Result:=true end
    end
    else begin
      kx:=dx/dy; ky:=dy/dx;

      t1.x:=x1; t1.y:=a.Y+(x1-a.X)*ky;
      t2.x:=x2; t2.y:=a.Y+(x2-a.X)*ky;
      ...
      if t2.y > lt.y then begin
        ...
        if y1 < p1.Y then y1:=p1.Y;
        if y2 > p2.Y then y2:=p2.Y;

        if t1.y < y1 then begin
          t1.x:=a.X+(y1-a.Y)*kx; t1.y:=y1
        end;
        if t2.y > y2 then begin
          t2.x:=a.X+(y2-a.Y)*kx; t2.y:=y2
        end;
...
function Rotate_vpoint(const a,b,c: TPoint; const v: vpoint): vpoint;
var
  p: TPoint;
begin
