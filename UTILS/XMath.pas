...
function win_GSmoothF(win: PFloats; r: int; s: float): int;
function win_GSmoothD(win: PDoubles; r: int; s: float): int;

procedure gauss_Singles(dst,src: PSingles; N,r: int; Sigma: Single);

function single_range(vp: PSingles; vn: int; out vmin,vmax: Single): int;
...
function Solve_projective1(p,q: PGPoly; out T1,T2: Real3x3): Boolean;
var
  i: int; n1,n2,_n1,_n2,_t1,_t2: Real3x3;
begin
  Result:=false;
...
function win_GSmoothF(win: pfloats; r: int; s: float): int;
var
  i,r2: int; x,d,k,w,ws: Double;
begin
  Result:=0; r2:=r+r;

  if s >= 0.01 then
  if (r > 0) and (r <= 100) then begin
    ...
    for i:=0 to r2 do win[i]:=win[i]/ws;
...
begin
  ...
  if win_GSmoothF(@vk,r,Sigma) > 0 then begin
