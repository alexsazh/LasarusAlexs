unit XVirts; interface

uses
  Windows,SysUtils,Classes,
  ActiveX,Math,OTypes,XDos;
...
type
  ...
  TVirtMem = class
    ...
    function vm_CompBuf(inp,dst: longint; VM: TVirtMem): bool;
    ...
    function Stm_Paste(p: int; const stm: IStream; pos: int): int;
    function Stm_Comp(p: int; const stm: IStream; pos: int): bool;
    ...
    function vm_Push(buf: Pointer; len: int): int;
    function vm_Pop(buf: Pointer; Size: int): int;

    function vm_Get(pos: int; buf: Pointer; Size: int; top: pint): int;
    procedure vm_Skip;

    procedure min_max_long(Ind,Count: int; out v1,v2: int);
    procedure min_max_inc(Ind,Count: int; out v1,v2: int);
    function min_max_xy(Ind,Count: int; out lt,rb: TPoint): int;
    function min_max_dxy(Ind,Count: int; out lt,rb: TPoint): int;
...
var
  pg,top,cbRead: longint; pos: Largeuint;
begin
  pg:=ofs div pg_Size;
  top:=pg * pg_Size;
...
var
  cbWritten: Longint; Pos: Largeuint;
begin
...
begin
  ...
  or (StrIComp(vm_Path,Path) <> 0) then begin
    ...
    if StrToWideChar(Path,fn,255) <> nil then begin
      ...
      rc:=vm_Storage.OpenStream(fn,nil,mode,0, vm_Stream);
      if rc = S_OK then begin
...
function TVirtMem.vm_ResizeBuf1(p, len: int): int;
var
  Hdr: vm_Len;
begin
  Result:=0;
...
function TVirtMem.vmx_BufSize(p: longint): Integer;
var
  Hdr: vmd_Len;
begin
  Result:=0;
...
function TVirtMem.vm_UpdateMsg(p: longint; const Msg: string): longint;
var
  len,len1: int; s: String;
begin
  ...
  if len1 = 0 then begin
    ...
    if len < len1 then begin
      if len > 0 then vm_FreeMem(p,len+2);
      vm_GetMem(p,len1+2);
      vm_Store(p,len1,1)
    end;

    vm_Store(p+1,s,len1+1)
  end;

  Result:=p
end;

function TVirtMem.vm_CompBuf(inp,dst: longint; VM: TVirtMem): bool;
var
  len1,len2,p1,p2,cx: int; buf1,buf2: TBytes;
begin
  Result:=true;

  p1:=vmx_Block(inp,len1);
  p2:=VM.vmx_Block(dst,len2);

  if len1 <> len2 then
    Result:=false
  else
  if len1 > 0 then
  while len1 > 0 do begin cx:=len1;
    if cx > Sizeof(buf1) then cx:=Sizeof(buf1);

    p1:=vm_Load(p1,buf1,cx);
    p2:=VM.vm_Load(p2,buf2,cx);

    if not CompareMem(@buf1,@buf2,cx) then begin
      Result:=false; Break
    end;
...
var
  s: string; len,slen: int;
begin
  Result:=0;

  if inp > 0 then begin
    slen:=0;
    vm_Load(inp+1,slen,1);
    vm_Load(inp+1,s,slen+1);

    len:=(slen+2+3) and $FFFC; Dec(len,2);

    Result:=VM.vm_Ind; VM.vm_Store(Result,len,1);
    VM.vm_Store(Result+1,s,slen+1);
    Inc(VM.vm_Ind,len+2)
  end
end;
...
begin
  bp:=@Msg;
  Result:=vm_Load(Adr,bp[0],1);
  len:=bp[0]; if len > 0 then
  Result:=vm_Load(Result,bp[1],len)
end;
...
function TVirtMem.Stm_Comp(p: int; const stm: IStream; pos: int): bool;
var
  len1,len2,p1,cx: int; buf1,buf2: tbytes;
begin
  Result:=false;

  if (p > 0) and (pos >= 0) then begin

    if xSeek(stm,pos) then
    if xRead(stm,len2,SizeOf(len2)) then begin
      p1:=vmx_Block(p,len1);
...
function TVirtMem.vm_Push(buf: Pointer; len: int): int;
var
  pos: int;
begin
  Result:=0;
  if vm_Active then begin
    ...
    vm_Append(pos,4); Result:=vm_Ind-4
  end
end;

function TVirtMem.vm_Pop(buf: Pointer; size: int): int;
var
  top: int;
begin
  Result:=vm_Get(vm_Ind-4,buf,size,@top);
  if Result > 0 then
  if Assigned(buf) then
  vm_Ind:=top
end;

function TVirtMem.vm_Get(pos: int; buf: Pointer; Size: int; top: pint): int;
var
  bx,cx: int;
begin
  Result:=0;
  if vm_Active then
  if pos+4 <= vm_Ind then begin

    vm_Load(pos,bx,4);
    if bx >= 0 then
    if bx+4 < pos then begin
