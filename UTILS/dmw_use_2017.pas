...
type
  ...
  TOnObject = function(ptr: Integer;
                       const dRec: dm_Rec): Boolean of object;

  TOnObject1 = function(ptr,lev: Integer;
                        const dRec: dm_Rec): Boolean of object;
...
function dm_Enabled: Integer; stdcall;
...
procedure dm_Put_sys(pps,prj,elp: Integer;
                     b1,b2,lc: Double;
                     dx,dy,dz: Integer); stdcall;

function dm_Get_sys7(out sys: tsys): Integer; stdcall;
...
function dm_Get_Stat(Stat: PIntegers;
                     StatMax: Integer): Integer; stdcall;

procedure dm_Put_Stat(Stat: PIntegers; Count: Integer); stdcall;
...
procedure dm_Put_Str(n: Integer; s: PChar);
...
function dm_enum_Indexof(bln,nnn: Integer; str: PChar): Integer; stdcall;
...
function dm_Get_Unicode1(n: word; s: PWideChar; maxLen: Integer): Boolean; stdcall;
...
// Изменить габарит карты <x1,y1,x2,y2> метры
...
function dm_rebound(extend: bool): int; stdcall;

function dm_Cut_Polygon(dst: PChar; lp: PLLine): Boolean; stdcall;

procedure dm_Poly_params(lp: PLPoly; N: Integer;
                         out s,l: Double); stdcall;
...
function dm_Seek_Blank1(Ind: int): int;
external dll_dm;

function dm_Seek_attr1(nn: int): PAbcRec1;
external dll_dm;
...
function dm_Get_Unicode1(n: word; s: PWideChar; maxLen: Integer): Boolean;
external dll_dm;
...
procedure dm_close_idx; stdcall; external dll_dm;
...
procedure dmw_load_palette(nm: PChar);
external dll_dm;

function dmw_get_rgb(var r,g,b: byte): Longint;
external dll_dm;
...
function merc_Create(Dest,Obj: PChar;
                     bmin,lmin,bmax,lmax: Double;
                     bc: Double; elp,Scale: Integer): Boolean;
external dll_dm;
...
function dm_rebound(extend: bool): int; external dll_dm;
