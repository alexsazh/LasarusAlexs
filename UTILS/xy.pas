...
function Square4(a,b,c,d: double): double;

procedure Max_Gauss_Bound(G: PGPoly; N: int; out lt,rb: tgauss);
...
function gauss_Square(G: PGPoly; N: int): double;

function gauss_Polygon(G: PGPoly; n: int; out c: TGauss): double;

function gauss_Centre(G: PGPoly; n: int): TGauss;
function gauss_xCentre(G: PGPoly; n: int): TGauss;
...
function xy_locate_frame(fr,loc: PGPoly; k: double): int;
...
procedure TPolygonSquare.Reset;
begin
  dn:=false; sqr:=0;
end;

procedure TPolygonSquare.LineTo(x,y: double);
begin
  if dn then
  sqr:=sqr + xpos*y - x*ypos;
  dn:=true; xpos:=x; ypos:=y;
end;

function TPolygonSquare.Square: double;
begin
  Result:=sqr / 2
end;
...
begin
  ...
  si:=@G[0]; di:=@G[i2]; bx:=Sizeof(TGauss);
...
function xy_locate_frame(fr,loc: PGPoly; k: double): int;
var
  i: int; lt,rb,p: TGauss; l: LOrient;
begin
  Result:=-1; lt:=loc[0]; rb:=loc[1];

  for i:=0 to 3 do begin p:=fr[i];
  ...
  if (Result < 0) and (k > 0) then begin
    for i:=0 to 3 do begin
      l[i].X:=Round(fr[i].x*k);
      l[i].Y:=Round(fr[i].y*k);
    end; l[4]:=l[0];

    p.x:=(lt.x+rb.x)*k/2;
    p.y:=(lt.y+rb.y)*k/2;

    if _rPolygonContainsPixel(@l,4,p.x,p.y,nil) = 1 then
    Result:=4
  end
end;
