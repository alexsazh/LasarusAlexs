...
2: (r1,r2,r3, r4,r5,r6, r7,r8,r9: Double);
  ...
  XSpace = record
    mode: int; o: txyz; e: TSpace
  end;

  pcube = ^tcube;
  tcube = record case int of
  0: (xmin,ymin,zmin, xmax,ymax,zmax: Double);
  1: (vmin,vmax: txyz)
  end;

  pcubef = ^tcubef;
  tcubef = record case int of
  0: (xmin,ymin,zmin, xmax,ymax,zmax: float);
  1: (vmin,vmax: txyzf)
  end;

  pcubef_arr = ^tcubef_arr;
  tcubef_arr = Array[0..1023] of tcubef;
...
function Matrixf(const M: TMatrix): TMatrixf;
function Matrix3f(const M: TMatrix): TMatrix3f;
...
function transpose_matrix(const Tr: TMatrix): TMatrix;
function transpose_matrix3f(const Tr: TMatrix): TMatrix3f;
...
function Line_Matrix(out Tr: TMatrix; const a,b: txyz): Boolean;

function iLine_Matrix(out Tr: TMatrix; const a,b: txyz): Boolean;

function lRotate_Matrix(out Tr: TMatrix; const a,b,c: txyz): Boolean;
...
procedure xyzv_matrix(v: pxyz_array; n: int; const T: TMatrix);

function xyz_proj(x,y,z: Double; const T: TMatrix): txyz;
function xyzv_proj(const v: txyz; const T: TMatrix): txyz;

procedure xyzv_min_max(vp: pxyz_array; n: int; out vmin,vmax: txyz);

procedure xyzv_min_maxp(vp: Pointer; n,stride: int;
                        out vmin,vmax: txyz);

function xyzv_centre(vp: pxyz_array; vn: int): txyz;

function Bound_to_Volume(const lt,rb: txyz; V: pxyz_array): int;

function cube_identify: tcube;
function cubef_identify: tcubef;

procedure max_cube(cube: pcube; v: pxyz);
procedure add_cube(cube1,cube2: pcube);

procedure add_cubef(var cube1: tcubef; const cube2: tcubef);

function cubef_to_cube(const c: tcubef): tcube;

function max_dist_to_cube(const v: txyz; const c: tcube): Double;

function cube_center(const c: tcube): txyz;
function cube_size(const c: tcube): txyz;
function cube_radius(const c: tcube): double;
...
procedure xyz_min(var a: txyz; const b: txyz);
...
function xyz_radius(const a,b: txyz): double;
...
function ilpz(const ln: tline3; z: double; out v: txyz): Boolean;

function ilp3(const ln: tline3;
              const pl: tplane3;
              out v: txyz): Boolean;

function ivpt(const a,b, c,n: txyz; out v: txyz): Boolean;
...
var
  f: double;
begin
  f:=x*T[3,0] + y*T[3,1] + z*T[3,2] + T[3,3];
  Result.x:=(x*T[0,0] + y*T[0,1] + z*T[0,2] + T[0,3])/f;
  Result.y:=(x*T[1,0] + y*T[1,1] + z*T[1,2] + T[1,3])/f;
  Result.z:=(x*T[2,0] + y*T[2,1] + z*T[2,2] + T[2,3])/f;
end;
...
procedure xyzv_min_maxp(vp: Pointer; n,stride: int;
                        out vmin,vmax: txyz);
var
  i,bx: int; v,v1,v2: txyz;
begin
  bx:=stride;
  if bx = 0 then bx:=sizeOf(txyz);

  v1:=pxyz(vp)^; v2:=v1;

  for i:=1 to n-1 do begin
    Inc(TPointer(vp),bx); v:=pxyz(vp)^;
...
procedure add_cubef(var cube1: tcubef; const cube2: tcubef);
begin
  if cube1.xmin > cube1.xmax then
    cube1:=cube2
  else with cube1 do begin
    xmin:=Min(xmin,cube2.xmin); xmax:=Max(xmax,cube2.xmax);
    ymin:=Min(ymin,cube2.ymin); ymax:=Max(ymax,cube2.ymax);
    zmin:=Min(zmin,cube2.zmin); zmax:=Max(zmax,cube2.zmax);
  end
end;

function cubef_to_cube(const c: tcubef): tcube;
begin
  with c.vmin do Result.vmin:=_xyz(x,y,z);
  with c.vmax do Result.vmax:=_xyz(x,y,z);
end;
...
function cube_center(const c: tcube): txyz;
begin
  Result:=xyz_centre(c.vmin,c.vmax)
end;

function cube_size(const c: tcube): txyz;
begin
  Result:=xyz_sub(c.vmax,c.vmin)
end;

function cube_radius(const c: tcube): double;
begin
  Result:=Max(Max(c.xmax-c.xmin,c.ymax-c.ymin),c.zmax-c.zmax)
end;
...
function xyz_abs(const v: txyz): txyz;
begin
  Result:=_xyz(abs(v.x),abs(v.y),abs(v.z))
end;
...
function solve_plane(out pl: tplane3; vp: pxyz_array; vn: int): Boolean;
var
  i: int; c: txyz; m,s1,s2,a: txyz3;
  sx,sy,sxy,sx2,sy2,sxz,syz,sz: Extended;
  v: pxyz; vx,vy,vz: double; b: txyz;
begin
  Result:=false;
  ...
  if vn > 3 then begin
    ...
    sx:=0; sy:=0; sz:=0; sxy:=0; sx2:=0; sy2:=0; sxz:=0; syz:=0;

    v:=Pointer(vp);
    for i:=1 to vn do begin
      vx:=v.x; vy:=v.y; vz:=v.z; Inc(v);
...
function izp3(const pl: tplane3; nz: double; out vz: double): Boolean;
var
  t: Double;
begin
  Result:=false;
